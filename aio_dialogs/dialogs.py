...
from aio_dialogs.aio_services import action_select, correct_action_input, uncorrect_action_input
...
new_action_dialog = Dialog(
    Window(
        Const('Создание нового действия'),
        Const('Выберите или напишите'),
        Group(Select(
            Format('{item[0]}'),
            id='action',
    ...
    Window(
        Const('Действие сохранено'),
        state=new_action.save
    )
)
