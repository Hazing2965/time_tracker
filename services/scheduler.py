    ...
    # Сравниваем прошло ли 24 часа с момента начала записи
        ...
        if time_start:
            ...
            if date_now - time_start >= timedelta(hours=24):
                # Если прошло, то получаем user_id и делаем ему /stop уведомляя что прошло больше 24 часов с момента открытия действия
                user_id = record.get('user_id')
                if user_id:
                    try:
                        await bot.send_message(user_id, 'Уже прошло больше 24 часов после начала действия, запись завершается в автоматическом режиме')
                        await stop_record(bot, user_id)
                    except Exception as e:
                        await bot.send_message(ADMIN_ID, f'Ошибка при автоматическом завершении записи у пользователя: {user_id}\n\n{e}')
